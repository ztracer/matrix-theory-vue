...
- **直觉化图解**：第 3 讲新增对角化相似变换图与“换汇贩卖机”类比，辅助理解 `P^{-1}AP` 的右乘/左乘顺序
- **课程目录**：右侧 sticky TOC 自动追踪阅读位置，可折叠收纳
- **访问进度**：滚动进度条 + localStorage 已访问标记 + 首页卡片 ✓ 标识 + 学习路径"继续"导航 + 可重置访问记录
- **键盘导航**：`←` `→` 翻课快捷键
- **知识图谱**：首页按小知识点展示前置依赖、推导路径和考试关联
- **暗色模式**：手动切换 + 跟随系统偏好，localStorage 持久化
- **真题例题详解**：34 道历年真题，独立汇总页按知识点分类检索
- **课后作业**：62 道教材习题精解，按章节分组、知识点标签
- **考前预测**：25 道期末考模拟预测题，覆盖全部知识点，含计算题与证明题完整解答
- **矩阵行内渲染**：含中文的混合行中矩阵自动以行内公式渲染，避免孤立的展示块
...
| 03 | 对角化与 Jordan 标准形 | 特征值特征向量、代数/几何重数、对角化图解与换汇类比、Jordan 块与链、C-H 定理 |
...
| 考前预测 | `/preview` | 25 道期末考预测题，覆盖全部知识点 |
...
│   │   └── router.test.js
│   ├── assets/                    # 静态资源
│   │   └── diagonalization.png    # 第 3 讲对角化相似变换图解
│   ├── components/
