...
矩阵论可视化交互学习系统是一个面向研究生矩阵论课程的交互式教学网站。项目采用 Vue 3 + Vite 技术栈，将抽象的矩阵论概念通过 SVG/Canvas/WebGL 动画进行可视化呈现，帮助学习者从几何直观角度深入理解线性空间、线性变换、矩阵分解、广义逆等核心知识点。每个知识模块均配备交互动画演示、定理公式推导、真题例题详解和课后作业，实现"概念-动画-定理-真题-作业"的闭环学习体验。
...
| Three.js | ^0.185 | WebGL 3D 向量场景，支持拖拽旋转与缩放（Gram-Schmidt、投影可视化） |
| SVG/Canvas | - | 原生矢量动画与二维交互演示 |

## 功能特性

- **11 个知识模块**：线性空间 → 考前复习，完整覆盖矩阵论课程
- **35+ 交互动画**：每个知识点配备可交互的可视化演示，含 3 个 Three.js 3D 可拖拽场景
- **知识图谱导航**：首页按小知识点展示前置依赖、推导路径和考试关联
...
| 04 | 矩阵函数与矩阵微分方程 | 矩阵幂级数收敛性、谱半径、对角化/Jordan块/旋转矩阵三种 e^At 计算入口、插值条件动画、核心题型精讲 |
...
| 06 | SVD 奇异值分解 | 奇异值分解 A=UΣV^T、SVD 几何意义（旋转-拉伸-旋转）、Frobenius 范数、低秩逼近 |
| 07 | 满秩分解与 MP 逆 | 满秩分解 A=FG、MP逆的意义（最小二乘+极小范数）、Penrose方程、对角化/Jordan/SVD构造、公式记忆方法 |

### 第 3 周：应用专题与真题

| 编号 | 知识点 | 核心内容 |
|------|--------|----------|
| 08 | 投影矩阵与应用 | 投影矩阵 P²=P、正交投影 vs 斜投影、R³ 3D投影可视化、矩阵方程 AXB=D |
